...
const calcHoursFromTimes = (tin, tout) => {
  const diff = timeToMinutes(tout) - timeToMinutes(tin);
      ...
      <div className="wrap" style={{ maxWidth: 560, margin: "0 auto", padding: "72px 32px 120px" }}>
        ...
        <div>
          ...
          {sorted.length === 0 ? (
            ...
            <div key={e.id}>
              {editingId === e.id ? (
                <EditRow entry={e} onSave={saveEdit} onCancel={() => setEditingId(null)} dark={dark} />
              ) : (
                <div className={`log-item${removing === e.id ? " out" : ""}`}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, color: c.text }}>{fmtDate(e.date)}</div>
                    {e.timeIn && e.timeOut && (
